...
pub fn translate_ir_to_mir(ir_module: &mut Module) -> MirModule {
    ir_pass::critical_edge::break_critical_edges(ir_module);
    let (copy_map, cfgs) = CopyMap::new_and_cfg(ir_module);
    let mut mir_module = mirgen::codegen_ir_to_mir(ir_module, copy_map, cfgs);
...
mod testing {
    ...
    fn test_translate_ir_to_mir() {
        ...
        let mut ir_module = builder.module;
        let mir_module = translate_ir_to_mir(&mut ir_module);
