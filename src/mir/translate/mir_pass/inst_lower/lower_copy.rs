...
pub fn lower_copy32_inst(copy32: &MirCopy32, out_actions: &mut VecDeque<LowerInstAction>) {
    ...
    match src {
        MirOperand::GPReg(gpreg) => {
            ...
            copy_from_reg(dst, pure, out_actions);
        }
        MirOperand::VFReg(vfreg) => {
            let pure = DispatchedReg::from_reg(vfreg.into());
            copy_from_reg(dst, pure, out_actions);
        }
        MirOperand::Imm64(imm64) => copy_from_u64(dst, imm64.get_value(), out_actions),
        MirOperand::Imm32(imm32) => copy_from_u32(dst, imm32.get_value(), out_actions),
        MirOperand::F32(f) => copy_from_u32(dst, f.to_bits(), out_actions),
        MirOperand::F64(f) => copy_from_u64(dst, f.to_bits(), out_actions),
    ...
    fn copy_from_reg(dst: GPR32, src: DispatchedReg, out_actions: &mut VecDeque<LowerInstAction>) {
        use DispatchedReg::*;
        let inst = match src {
    ...
    fn copy_from_u64(dst: GPR32, src: u64, out_actions: &mut VecDeque<LowerInstAction>) {
    ...
    fn copy_from_u32(dst: GPR32, src: u32, out_actions: &mut VecDeque<LowerInstAction>) {
...
pub fn lower_copy64_inst(copy64: &MirCopy64, out_actions: &mut VecDeque<LowerInstAction>) {
    ...
    match src {
        ...
        MirOperand::F64(f) => copy_from_u64(dst, f.to_bits(), out_actions),
        MirOperand::PState(_) => todo!("MSR and MSR instructions have not been implemented yet."),
        MirOperand::Label(label_ref) => copy_from_label(dst, label_ref, out_actions),
        MirOperand::Global(global_ref) => copy_from_global(dst, global_ref, out_actions),
        MirOperand::SwitchTab(switch_tab) => copy_from_switch_tab(dst, switch_tab, out_actions),
        MirOperand::None => { /* Nothing */ }
    }

    fn copy_from_reg(dst: GPR64, src: DispatchedReg, out_actions: &mut VecDeque<LowerInstAction>) {
        use DispatchedReg::*;
        let inst = match src {
    ...
    fn copy_from_u64(dst: GPR64, src: u64, out_actions: &mut VecDeque<LowerInstAction>) {
        let inst = if imm_traits::is_mov_imm(src) {
    ...
    fn copy_from_u32(dst: GPR64, src: u32, out_actions: &mut VecDeque<LowerInstAction>) {
        let inst = if imm_traits::is_mov_imm(src as u64) {
    ...
    fn copy_from_symbol(dst: GPR64, src: MirSymbolOp, out_actions: &mut VecDeque<LowerInstAction>) {
        let inst = LoadConst64Symbol::new(MirOP::LoadConst64Symbol, dst, src);
        out_actions.push_back(LowerInstAction::NOP(inst.into_mir()));
    }
    fn copy_from_label(dst: GPR64, src: MirBlockRef, out_actions: &mut VecDeque<LowerInstAction>) {
        copy_from_symbol(dst, MirSymbolOp::Label(src), out_actions);
    }
    fn copy_from_global(
        dst: GPR64,
        src: MirGlobalRef,
        ...
        copy_from_symbol(dst, MirSymbolOp::Global(src), out_actions);
    }
    fn copy_from_switch_tab(
        dst: GPR64,
        src_index: u32,
        out_actions: &mut VecDeque<LowerInstAction>,
    ) {
        copy_from_symbol(dst, MirSymbolOp::SwitchTab(src_index), out_actions);
    }
}
...
pub fn lower_fcopy32_inst(
    ...
    match src {
        MirOperand::GPReg(src) => {
            ...
            copy_from_reg(dst, src, out_actions)
        }
        MirOperand::VFReg(src) => {
            let src = DispatchedReg::from_reg(src.into());
            copy_from_reg(dst, src, out_actions)
        }
        MirOperand::Imm64(src) => copy_from_u64full(dst, src.get_value(), vreg_alloc, out_actions),
        MirOperand::Imm32(src) => {
            copy_from_u64full(dst, src.get_value() as u64, vreg_alloc, out_actions)
        }
        MirOperand::F32(src) => copy_from_f32(dst, src, vreg_alloc, out_actions),
        MirOperand::F64(src) => copy_from_f32(dst, src as f32, vreg_alloc, out_actions),
    ...
    fn copy_from_f32(
        ...
        if imm_traits::try_cast_f32_to_aarch8(src).is_some() {
            ...
            copy_from_u64full(dst, src.to_bits() as u64, vreg_alloc, out_actions);
        }
    }
    fn copy_from_u64full(
        ...
        out_actions.push_back(LowerInstAction::NOP(load_to_temp.into_mir()));
        let vreg32 = GPR32(vreg.0, vreg.1);
        let inst = UnaFG32::new(MirOP::FMovFG32, dst, vreg32);
        out_actions.push_back(LowerInstAction::NOP(inst.into_mir()));
    }
    fn copy_from_reg(dst: FPR32, src: DispatchedReg, out_actions: &mut VecDeque<LowerInstAction>) {
        use DispatchedReg::*;
        let inst = match src {
...
pub fn lower_fcopy64_inst(
    ...
    match src {
        ...
        MirOperand::F32(src) => copy_from_f64(dst, src as f64, vreg_alloc, out_actions),
        MirOperand::F64(src) => copy_from_f64(dst, src, vreg_alloc, out_actions),
    ...
    fn copy_from_f64(
        ...
        if imm_traits::try_cast_f64_to_aarch8(src).is_some() {
            ...
            copy_from_u64full(dst, src.to_bits(), vreg_alloc, out_actions);
        }
    }
    fn copy_from_u64full(
        dst: FPR64,
    ...
    fn copy_from_reg(dst: FPR64, src: DispatchedReg, out_actions: &mut VecDeque<LowerInstAction>) {
        use DispatchedReg::*;
        let inst = match src {
