...
pub const fn is_smax_imm(imm: i64) -> bool {
    // SMax 指令的立即数是一个 8 位的有符号整数
    imm >= -128 && imm <= 127
}

pub const fn is_umax_imm(imm: u64) -> bool {
    // UMax 指令的立即数是一个 8 位的无符号整数
    imm <= 255
}

pub const fn is_shift_imm(imm: u64) -> bool {
    // Shift 指令的立即数是一个 6 位的无符号整数
    imm <= 63
}
...
pub const fn is_mov_imm(imm: u64) -> bool {
    // 检查是否是一个 16 位的整数, 通过无符号扩展得到原数
    (imm & 0xFFFF) == imm
}

/// 检查该 64 位立即数是否满足 `ldr r, [r, #i]` 等指令的立即数条件;
/// 具体来说, 立即数应该是一个 9 位的整数, 包含 simm 和 pimm 两种模式.
///
/// `simm`: 范围在 -256 到 255 之间的有符号整数.
/// `pimm` (32位指令): 范围在 0-16380 之间的无符号整数, 且必须是 4 的倍数.
/// `pimm` (64位指令): 范围在 0-32760 之间的无符号整数, 且必须是 8 的倍数.
pub const fn is_load_imm(imm: i64) -> bool {
