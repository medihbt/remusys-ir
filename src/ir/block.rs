...
use mtb_entity_slab::{
    ...
    IEntityAllocID, IEntityListNodeID, IPoliciedID, OrderCachedList, entity_id,
};
use std::cell::Cell;
...
#[entity_id(BlockID, policy = 256, allocator_type = BlockAlloc, backend = index)]
pub struct BlockObj {
...
impl BlockID {
    ...
    pub fn try_deref_ir(self, allocs: &IRAllocs) -> Option<&BlockObj> {
        self.inner().try_deref(&allocs.blocks)
    }
    pub fn is_alive(self, allocs: &IRAllocs) -> bool {
        match self.try_deref_ir(allocs) {
            Some(b) => !b.dispose_mark.get(),
            None => false,
        }
    }
    pub fn try_get_entity_index(self, allocs: &IRAllocs) -> Option<usize> {
        self.try_deref_ir(allocs).map(|_| self.inner().get_order())
    }
    pub fn get_entity_index(self, allocs: &IRAllocs) -> usize {
        self.try_get_entity_index(allocs)
            .expect("Attempted to get entity index of dead or non-existent BlockID")
    }
