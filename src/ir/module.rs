...
pub struct ModulAllocatorInner {
    pub(super) _global_alloc: Slab<GlobalData>,
    pub(super) _expr_alloc: Slab<ConstExprData>,
    pub(super) _inst_alloc: Slab<InstData>,
    pub(super) _block_alloc: Slab<BlockData>,
}

impl Module {
    pub fn new(name: String, type_ctx: Rc<TypeContext>) -> Self {
        let inner = ModulAllocatorInner {
            _global_alloc: Slab::with_capacity(1024),
            _expr_alloc: Slab::with_capacity(1024),
            _inst_alloc: Slab::with_capacity(1024),
            _block_alloc: Slab::with_capacity(1024),
        };
        Self {
            name,
            type_ctx,
            ...
            _alloc_use: RefCell::new(Slab::with_capacity(1024)),
        }
    }
    ...
    pub fn insert_global(&self, data: GlobalData) -> GlobalRef {
        ...
        let id = inner._global_alloc.insert(data);
    ...
    pub fn get_expr(&self, expr: ConstExprRef) -> Ref<ConstExprData> {
        let inner = self.borrow_value_alloc();
        Ref::map(inner, |inner| {
            inner._expr_alloc.get(expr.get_handle()).unwrap()
        })
    }
    pub fn mut_expr(&self, expr: ConstExprRef) -> RefMut<ConstExprData> {
        let inner = self.borrow_value_alloc_mut();
        RefMut::map(inner, |inner| {
            inner._expr_alloc.get_mut(expr.get_handle()).unwrap()
        })
    }
    pub fn insert_expr(&self, data: ConstExprData) -> ConstExprRef {
        let mut inner = self.borrow_value_alloc_mut();
        let id = inner._expr_alloc.insert(data);
    ...
    pub fn get_inst(&self, inst: InstRef) -> Ref<InstData> {
        let inner = self.borrow_value_alloc();
        Ref::map(inner, |inner| {
            inner._inst_alloc.get(inst.get_handle()).unwrap()
        })
    }
    pub fn mut_inst(&self, inst: InstRef) -> RefMut<InstData> {
        let inner = self.borrow_value_alloc_mut();
        RefMut::map(inner, |inner| {
            inner._inst_alloc.get_mut(inst.get_handle()).unwrap()
        })
    }
    pub fn insert_inst(&self, data: InstData) -> InstRef {
        let mut inner = self.borrow_value_alloc_mut();
        let id = inner._inst_alloc.insert(data);
        InstRef::from_handle(id)
    }

    pub fn get_block(&self, block: BlockRef) -> Ref<BlockData> {
        let inner = self.borrow_value_alloc();
        Ref::map(inner, |inner| {
            inner._block_alloc.get(block.get_handle()).unwrap()
        })
    }
    pub fn mut_block(&self, block: BlockRef) -> RefMut<BlockData> {
        let inner = self.borrow_value_alloc_mut();
        RefMut::map(inner, |inner| {
            inner._block_alloc.get_mut(block.get_handle()).unwrap()
        })
    }
    pub fn insert_block(&self, data: BlockData) -> BlockRef {
        let mut inner = self.borrow_value_alloc_mut();
        let id = inner._block_alloc.insert(data);
